...
def create_candidate_pairs(df: pd.DataFrame, strategy: str = 'lastname_state',
                          window: int = 5) -> pd.MultiIndex:
    """
...
def evaluate_blocking_recall(pairs: pd.MultiIndex, ground_truth: pd.DataFrame,
                             record_id_mapping: pd.DataFrame) -> dict:
    """
    ...
    logger.info(f"Blocking recall: {recall:.2%} ({true_pairs_found}/{len(true_pairs)} true pairs found)")
...
def analyze_blocking_statistics(df: pd.DataFrame, field: str) -> dict:
    ...
    stats = {
        ...
        'avg_block_size': len(df) / df[field].nunique() if df[field].nunique() > 0 else 0
    }
