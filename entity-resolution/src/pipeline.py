...
from .data_loader import load_data_for_matching, create_record_id
from .blocking import create_candidate_pairs, evaluate_blocking_recall
from .comparison import build_comparison_features, add_composite_features
from .classification import classify_pairs
from .golden_record import create_golden_records, build_match_clusters
from .evaluation import evaluate_matches, error_analysis, evaluate_golden_records, generate_confusion_matrix_report
...
def run_entity_resolution_pipeline(config: dict, output_dir: str = None,
...
def add_record_ids_to_ground_truth(ground_truth_df: pd.DataFrame,
...
def save_results(golden_records_df: pd.DataFrame, matches: pd.Series,
                features: pd.DataFrame, metrics: dict,
    ...
    # Save golden records
    golden_file = output_path / output_config.get('golden_records_csv', 'golden_records.csv')
    golden_records_df.to_csv(golden_file, index=False)
    logger.info(f"Saved golden records to {golden_file}")

    # Save predicted matches
    matches_file = output_path / output_config.get('matches_csv', 'predicted_matches.csv')
    ...
    if len(matches_df) > 0:
        for col in features.columns:
            if col not in ['is_match']:
                ...
                matches_df[col] = [features.loc[idx, col] if idx in features.index
                                  else features.loc[(idx[1], idx[0]), col] if (idx[1], idx[0]) in features.index
                                  else None
    ...
    metrics_file = output_path / output_config.get('metrics_json', 'evaluation_metrics.json')
